...
function MyOrders() {
    ...
    const getStatusTimeline = (status) => {
        const timelines = {
            ...
            COMPLETED: ['Order Placed', 'Processing', 'Completed'],
    ...
    const handleReorder = (items) => {
        ...
        const cartItems = items.map(item => ({ ...item, units: item.units }));
        ...
        <Container maxWidth="lg" sx={{ py: 4 }}>
                        ...
                        <TableBody>
                            {orders.map((order) => (
                                <TableRow key={order.orderId} sx={{ '&:hover': { backgroundColor: '#f9f9f9' } }}>
                                    ...
                                    <TableCell sx={{ color: order.status === 'COMPLETED' ? '#2e7d32' : order.status === 'CANCELLED' ? '#d32f2f' : order.status === 'PROCESSING' ? '#0288d1' : '#ed6c02' }}>
                                    ...
                                    <TableCell>
                                        <Button
                                            ...
                                            sx={{ textTransform: 'none', backgroundColor: '#549a54', '&:hover': { backgroundColor: '#108015' } }}
                                        >
                        ...
                        {detailsLoading ? (
                            ...
                            <>
                                ...
                                <Box sx={{ mb: 3 }}>
                                    ...
                                    <Typography variant="body2" sx={{ color: selectedOrder.status === 'COMPLETED' ? '#2e7d32' : selectedOrder.status === 'CANCELLED' ? '#d32f2f' : selectedOrder.status === 'PROCESSING' ? '#0288d1' : '#ed6c02' }}>
                                    ...
                                    {selectedOrder.items && selectedOrder.items.length > 0 ? (
                                        selectedOrder.items.map((item, index) => (
                                            <Box key={index} sx={{ mb: 1, p: 1, bgcolor: '#f5f5f5', borderRadius: 1 }}>
                                                <Typography variant="body2">
                                                    {item.productName || `Item ${index + 1}`} - Units: {item.units || 'N/A'}
                                                </Typography>
                                            </Box>
                                        ))
                                    ) : (
                                ...
                                <Box sx={{ mb: 3 }}>
                                    ...
                                    {getStatusTimeline(selectedOrder.status).map((step, index) => (
                                        <Box key={index} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                                            ...
                                            />
                                            <Typography variant="body2" sx={{ color: '#0478C0' }}>
                                ...
                                <Box sx={{ mb: 3 }}>
                                    <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 1 }}>
                                        ...
                                        <Typography component="span" variant="subtitle1" sx={{ fontWeight: 'normal', color: 'grey' }}>
                                            077 22 35 273
                                        </Typography>
                                    </Typography>
                                </Box>
            ...
            <Box sx={{ mt: 4, display: 'flex', justifyContent: 'center' }}>
                <Button
                    ...
                    sx={{ borderRadius: 8, px: 4, py: 1.2, textTransform: 'none', backgroundColor: '#549a54', '&:hover': { backgroundColor: '#108015' } }}
                >
                    Continue Shopping
                </Button>
            </Box>
